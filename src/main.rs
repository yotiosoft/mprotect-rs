...
fn child_regionguard_with_pkey_workloads() -> Result<(), RuntimeError> {
    ...
    let pkey: PkeyGuard<Unassociated> = PkeyGuard::new(PkeyPermissions::NoAccess).map_err(RuntimeError::MprotectError)?;

    let mut assoc = pkey.associate::<_, _, PkeyPermissions::ReadWrite>(&mut safe_mem).map_err(RuntimeError::MprotectError)?;
    {
    ...
    let mut assoc2 = pkey.associate::<_, _, PkeyPermissions::ReadWrite>(&mut safe_mem2).map_err(RuntimeError::MprotectError)?;
    {
